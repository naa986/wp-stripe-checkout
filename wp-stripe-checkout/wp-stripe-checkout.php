...
class WP_STRIPE_CHECKOUT {
    ...
    function loader_operations() {
        ...
        add_action('add_meta_boxes_wpstripeco_order', 'wpstripeco_order_meta_boxes');
...
function wp_stripe_checkout_payment_link_button_handler($atts) {
    ...
    $button_code .= '</form>';
    $button_code = apply_filters('wpsc_payment_link_button', $button_code, $atts);
    return $button_code;
}
...
function wp_stripe_checkout_session_button_handler($atts) {
    ...
    $currency_form_code = '<input type="hidden" name="item_currency" value="'.esc_attr($currency).'" />';
    $currency_input_code = '';
    $currency_input_code = apply_filters('wp_stripe_checkout_session_currency', $currency_input_code, $button_code, $atts);
    if(!empty($currency_input_code)){
        $currency_form_code = $currency_input_code;
    }
    $button_code .= $currency_form_code;
    if(!empty($success_url)){
    ...
    $button_code .= '</form>';
    $button_code = apply_filters('wp_stripe_checkout_session_button', $button_code, $atts);
    return $button_code;
}
